...
if ($idjogo === '1') {
    ...
    WHERE ff.id_jogos = 4 And id_torneio = $edicao And grupo_final = 0
    ...
    WHERE chess.id_jogos = 5 And id_torneio = $edicao And chess.categoria = 'M'
...
<?php     if ($idjogo == 1 || $idjogo == 2 || $idjogo == 3) {
    ...
    FROM rankingff ff
    INNER JOIN equipe eq ON ff.id_equipe = eq.id_equipe
    WHERE ff.id_jogos = 4 And id_torneio = $edicao And grupo_final = 1
    ORDER BY ff.grupo, ff.pontos DESC";
        ...
        <?php if ($idjogo === '5' ||  $dados['categoria'] == 'F') {  ?>
            <div class="tabela-container">
    ...
    ORDER BY chess.grupo, chess.pontosT DESC";
                ...
                $grp = '';
                $primeira = true;
                while ($dados = mysqli_fetch_assoc($retornofem)) {  // Usar $retornofem aqui
                    if ($grp != $dados['grupo']) {
                        $grp = $dados['grupo'];
